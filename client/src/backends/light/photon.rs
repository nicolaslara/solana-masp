...
impl PhotonClient {
    ...
    async fn rpc_call<T: for<'de> Deserialize<'de>>(
        ...
        let json: PhotonResponse<T> = response.json().await.map_err(|e| {
            PhotonError::InvalidResponse(format!("Failed to parse Photon response: {}", e))
        })?;
        ...
        json.result
            .ok_or_else(|| PhotonError::InvalidResponse("Photon RPC missing result".to_string()))
    }
    ...
    pub async fn get_validity_proof(
        ...
        let result: GetValidityProofResponse = self.rpc_call("getValidityProofV2", params).await?;
    ...
    pub async fn get_nullifier_validity_proof(
        ...
        self.get_validity_proof(&derived_address, address_tree)
            .await
    }
    ...
    pub async fn get_validity_proofs_batched(
        ...
        if nullifiers.is_empty() {
            return Err(PhotonError::InvalidResponse(
                "No nullifiers provided".to_string(),
            ));
        }
        ...
        for chunk in nullifiers.chunks(BATCH_SIZE) {
            ...
            let result: GetValidityProofResponse =
                self.rpc_call("getValidityProofV2", params).await?;

            let proof_result = result.value.ok_or_else(|| {
                PhotonError::InvalidResponse("No validity proof returned".to_string())
            })?;

            // Parse the compressed proof
            let compressed = proof_result.compressed_proof.ok_or_else(|| {
                PhotonError::InvalidResponse("No compressed proof in response".to_string())
            })?;
...
fn parse_compressed_proof(
    response: &CompressedProofResponse,
) -> Result<CompressedProof, PhotonError> {
    Ok(CompressedProof {
