...
#[tokio::test]
#[ignore]
async fn ultraplonk_pipeline_masp_transfer() -> Result<(), ProofSystemError> {
    ...
    let witness = target.join("masp_transfer.gz"); // nargo execute outputs <circuit_name>.gz
    ...
    // NOTE(security): We intentionally serialize the VK *without* G2_X here.
    //
    // - G2_X is not circuit-specific; it's an SRS parameter ("x · G2") for the
    //   Barretenberg BN254 trusted setup.
    // - Our verifier uses the hard-coded `STANDARD_G2_X` constant during the final pairing
    //   check (it does NOT read G2_X from VK bytes). This avoids letting untrusted VK bytes
    //   influence the pairing parameters.
    // - `ultraplonk_core::VerificationKey::from_onchain_bytes` therefore expects the 1632-byte
    //   Solidity-format VK *without* G2_X.
    //
    // Production assumption: proofs/VKs are generated with the same SRS that `STANDARD_G2_X`
    // corresponds to. If we ever switch SRS, we must update the verifier’s G2_X (or plumb it
    // in explicitly as trusted data).
...
/// Shield circuit pipeline: compile → witness → prove → verify
#[tokio::test]
#[ignore]
async fn ultraplonk_pipeline_masp_shield() -> Result<(), ProofSystemError> {
    if !tool_exists("nargo") || !tool_exists("bb") {
        eprintln!("Skipping: requires `nargo` + `bb` on PATH");
        return Ok(());
    }
    ...
    let target = circuit_dir.join("target");
    let circuit_json = target.join("masp_shield.json");
    let witness = target.join("masp_shield.gz"); // nargo execute outputs <circuit_name>.gz
    let vk_bb = target.join("vk.bin");
    let proof_bin = target.join("proof.bin");
    ...
    // 3) Convert VK to on-chain format (without G2_X - verifier expects 1632 bytes)
    let vk_bb_bytes = read(&vk_bb);
    let vk = BbVk::from_bb_bytes(&vk_bb_bytes)
        .map_err(|_| ProofSystemError::NotImplemented("failed to parse bb vk".to_string()))?;
    let vk_onchain_bytes = vk.to_onchain_bytes_without_g2();
    ...
    assert!(ok, "expected shield proof verification to succeed");
    Ok(())
}

/// Unshield circuit pipeline: compile → witness → prove → verify
#[tokio::test]
#[ignore]
async fn ultraplonk_pipeline_masp_unshield() -> Result<(), ProofSystemError> {
    if !tool_exists("nargo") || !tool_exists("bb") {
        eprintln!("Skipping: requires `nargo` + `bb` on PATH");
        return Ok(());
    }
    ...
    let target = circuit_dir.join("target");
    let circuit_json = target.join("masp_unshield.json");
    let witness = target.join("masp_unshield.gz"); // nargo execute outputs <circuit_name>.gz
    let vk_bb = target.join("vk.bin");
    let proof_bin = target.join("proof.bin");
    ...
    // 3) Convert VK to on-chain format
    let vk_bb_bytes = read(&vk_bb);
    let vk = BbVk::from_bb_bytes(&vk_bb_bytes)
        .map_err(|_| ProofSystemError::NotImplemented("failed to parse bb vk".to_string()))?;
    ...
    assert!(ok, "expected unshield proof verification to succeed");
    Ok(())
}
